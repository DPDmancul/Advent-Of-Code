...
isFeasible :: [Pattern] -> Design -> Bool
isFeasible _ [] = True
isFeasible patterns design = any isMatch patterns
 where
  isMatch pattern = isPrefixOf pattern design && isFeasible patterns (drop (length pattern) design)

main = interact $ show . length . uncurry (filter . isFeasible) . parse
 where
